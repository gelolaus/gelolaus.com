...
<template>
    <footer class="h-12 bg-hacker-gray/90 border-t border-gray-700 flex items-center px-2 z-[9999] gap-2 w-full absolute bottom-0 backdrop-blur-sm">
        ...
        <div class="flex-1 flex items-center gap-2 overflow-x-auto no-scrollbar">
            <div 
                ...
                class="px-3 py-1 rounded flex items-center gap-2 cursor-pointer transition-all border-b-2 select-none"
                :class="[
                    (win.zIndex >= store.activeZIndex && !win.isMinimized)
                    ...
                    isMobile ? 'min-w-0' : 'min-w-[120px] max-w-[200px]'
                ]" 
            >
                ...
                <span v-if="!isMobile" class="text-xs truncate font-mono pt-0.5" :class="{'opacity-50': win.isMinimized}">
                    {{ win.title }}
                </span>
            </div>
        </div>
    
        <div v-if="!isMobile" class="px-4 text-xs text-gray-400 font-mono select-none">
            {{ clock }}
        </div>
    </footer>
</template>